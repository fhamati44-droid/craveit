...
import { listPublicDeals, currentTier, nextTier, countdown, pad, tierProgress, sortTiers } from '@/lib/groupDealApi';
...
export default function Deals() {
  ...
  // Only show active and scheduled deals (not ended/draft)
  const visibleDeals = deals.filter(v => v.status === 'active' || v.status === 'scheduled');
  ...
  return (
    <div className="pt-4 pb-8">
      ...
      <div className="px-4 space-y-4">
        {visibleDeals.length ? (
          visibleDeals.map(v => <DealListCard key={v.deal.id} view={v} onOpen={() => navigate(`/deals/${v.deal.id}`)} />)
        ) : (
          <div className="text-center py-16 text-on-surface-variant">
            <p className="text-4xl mb-2">🎉</p>
            <p>ما في عروض جماعية هسا</p>
          </div>
        )}
      </div>
    </div>
...
function DealListCard({ view, onOpen }) {
  const { deal, thresholds, participants } = view;
  ...
  return (
    <button onClick={onOpen} className="block w-full text-right bg-surface-container border border-outline-variant/30 rounded-2xl overflow-hidden active:scale-[0.98] transition-transform">
      <div className="relative h-40">
        {deal.hero_image
          ? <img className="w-full h-full object-cover" src={resolvePublicImage(deal.hero_image)} alt={deal.title} onError={handleImageError} />
          : <div className="w-full h-full bg-surface-container-high flex items-center justify-center text-4xl">🎉</div>}
        <div className="absolute inset-0 bg-gradient-to-t from-surface-container via-transparent to-transparent" />
        {isScheduled ? (
          <div className="absolute top-2 left-2 bg-tertiary text-on-tertiary px-2 py-1 rounded-lg text-[10px] font-bold">عرض قادم</div>
        ) : cd && !cd.expired ? (
      ...
      <div className="p-4 space-y-3">
        ...
        {deal.restaurant_name_snapshot && <p className="text-[11px] text-on-surface-variant">{deal.restaurant_name_snapshot}</p>}
        <div className="flex items-end gap-2">
          {cur && <span className="text-primary text-2xl font-bold">₪{Math.round(cur.price)}</span>}
          {deal.reference_price != null && <span className="text-on-surface-variant line-through text-xs mb-1">₪{Math.round(deal.reference_price)}</span>}
        </div>
