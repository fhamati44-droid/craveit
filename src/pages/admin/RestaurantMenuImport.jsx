...
import {
  parseCsv, downloadCsvTemplate, searchTamamProducts, suggestMappings,
  getMenusForRestaurant, createMenu, createItem, createImportBatch,
...
export default function RestaurantMenuImport() {
  ...
  const doImport = async () => {
    ...
    try {
      ...
      const readyRows = [...groups.ready, ...groups.review.filter((v) => v.status === 'review')];
      ...
      for (const v of [...groups.ready, ...groups.review]) {
        ...
        const payload = {
          ...
          restaurant_product_name: d.restaurant_product_name || '', restaurant_sku: d.restaurant_sku || '',
          price: Number(d.price), compare_at_price: d.compare_at_price ? Number(d.compare_at_price) : null,
          currency: d.currency || 'ILS', tax_included: d.tax_included !== 'false',
          customer_visible_description: d.customer_visible_description || '', full_description_ar: d.full_description_ar || '',
          ...
          meal_id: d.meal_id ? Number(d.meal_id) : null, meal_name_snapshot: d.meal_name_snapshot || '',
          mapping_status: d.meal_id ? 'mapped' : 'unmapped', mapping_confidence: d.meal_id ? 100 : 0,
          active: true, available: true,
        };
        await createItem(payload);
        if (d.meal_id) created++; else unmapped++;
      }
      setResults({ created, unmapped, errors: groups.error.length, total: validated.length });
      setStep('done');
    ...
    <div dir="rtl" className="font-tamam max-w-2xl space-y-4">
      ...
      {step === 'upload' && (
        <div className="bg-surface-container rounded-2xl p-6 space-y-3">
          <button onClick={downloadCsvTemplate} className="flex items-center gap-1.5 text-sm bg-surface-container-high px-3 py-2 rounded-lg font-bold"><Download size={15} /> تحميل قالب CSV</button>
