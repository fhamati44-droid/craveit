...
/** Large feature meal card — used for اختيارات اليوم and غدا اليوم. */
export default function FeatureMealCard({ meal, badge }) {
  ...
  const openDetails = () => {
    track('homepage_meal_opened', { meal_id: meal.id, restaurant_id: meal.restaurant_id, card: 'feature' });
    navigate(mealRoute);
  };
  const handleAdd = (e) => {
    e.stopPropagation();
  ...
  return (
    <div className="flex-none w-72 bg-surface-container border border-outline-variant/30 rounded-2xl overflow-hidden flex flex-col">
      ...
      <div className="p-3 flex-1 flex flex-col">
        ...
        <div className="mt-3">
          {added ? (
            <button onClick={() => navigate('/cart')} className="w-full h-10 bg-surface-high text-on-surface rounded-xl text-xs font-bold flex items-center justify-center gap-1">
              <Icon name="check" className="text-[16px] text-primary" /> متابعة للسلة ({totalItems})
            </button>
          ) : (
