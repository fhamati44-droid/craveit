...
import { buildTiers, currentTier, nextTier, countdown, pad } from '@/lib/dealTiers';

const Icon = ({ name, className = '' }) => <span className={`material-symbols-outlined ${className}`}>{name}</span>;

export default function JoinedDealMiniBanner({ deal, participants, onOpen }) {
      ...
      <button onClick={onOpen} className="w-full bg-primary/10 border border-primary/30 rounded-2xl p-3 flex items-center justify-between active:scale-[0.99] transition-transform">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="text-right min-w-0">
            ...
            <div className="text-sm font-bold truncate">{deal.title || deal.name}</div>
