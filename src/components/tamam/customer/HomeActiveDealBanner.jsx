...
export default function HomeActiveDealBanner({ deal, onOpen }) {
      ...
      <div className="p-4 space-y-2">
        <div className="flex justify-between items-start">
          <div className="min-w-0">
            <h3 className="font-bold truncate">{deal.title || deal.name || 'عرض جماعي'}</h3>
          ...
          <div className="text-left flex-shrink-0">
            ...
            {deal.original_price != null && <div className="text-on-surface-variant line-through text-xs">₪{deal.original_price}</div>}
          </div>
        </div>
        <div className="w-full h-1.5 bg-surface-container rounded-full overflow-hidden"><div className="h-full bg-primary rounded-full" style={{ width: `${pct}%` }} /></div>
        <div className="flex justify-between text-[11px] font-bold">
          <span className="text-primary">{participants} مشتركين</span>
          <span className="text-on-surface-variant">{next ? `ناقص ${next.at - participants} والسعر ₪${Math.round(next.price)}` : 'وصلنا لأفضل سعر'}</span>
        </div>
        <div className="flex items-center justify-end gap-1 text-primary font-bold text-sm pt-1"><span>شوف العرض</span><Icon name="arrow_back" /></div>
      </div>
    </button>
  );
}